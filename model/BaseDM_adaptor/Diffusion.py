...
def extract(a, t, x_shape):
    ...
    # out = a.gather(-1, t)  # 在最后一个维度上根据 t 提取 a 中的元素
    # return out.reshape(b, *((1,) * (len(x_shape) - 1)))  # 将 out reshape 成特定形状
...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        # xt = sqrt(alpha_bar) * x + sqrt(1-alpha_bar) * noise
        return (
    ...
    def p_losses(self, x_start_cond, x_start_pred, cond_fea, t, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        if self.loss_type == 'l1':
            ...
            loss = F.mse_loss(noise*10, pred_noise*10)
        else:
            raise NotImplementedError()

        #预测去噪后的初始帧（真实的未来帧） x0=xt-sqrt(1-bar_alpha_t)*noise/sqrt(bar_alpha_t)
