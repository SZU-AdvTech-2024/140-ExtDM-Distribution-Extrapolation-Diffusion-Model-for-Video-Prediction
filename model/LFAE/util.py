...
import torch
from model.LFAE.sync_batchnorm import SynchronizedBatchNorm2d as BatchNorm2d
...
def make_coordinate_grid(spatial_size, type, device):
    ...
    """
    h, w = spatial_size
    x = torch.arange(w).type(type).to(device)
    y = torch.arange(h).type(type).to(device)
...
class DownBlock2d(nn.Module):
    ...
    def forward(self, x):
        out = self.conv(x.to(next(self.parameters()).device))
        out = self.norm(out)
        out = F.relu(out)
        out = self.pool(out)
        return out
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for down_block in self.down_blocks:
            outs.append(down_block(outs[-1].to(next(self.parameters()).device)).to(next(self.parameters()).device))
        return outs
    ...
    def forward(self, x):
        for i in range(len(x)):
            if torch.isnan(x[i]).any():
                x[i] = torch.where(torch.isnan(x[i]), torch.zeros_like(x[i]), x[i])
...
def to_homogeneous(coordinates):
    ...
    ones_shape[-1] = 1
    ones = torch.ones(ones_shape).type(coordinates.type()).to(coordinates.device)

    return torch.cat([coordinates, ones], dim=-1)


def from_homogeneous(coordinates):
    # print(f"Max value of coordinates: {coordinates.max()}")
    # print(f"Min value of coordinates: {coordinates.min()}")
    epsilon = 1e-10
    result = coordinates[..., :2] / (coordinates[..., 2:3] + epsilon)  # 添加epsilon避免除以零
    # result = torch.clamp(result, min=-1.0, max=1.0)  # 限制结果在[-1, 1]之间
    # return 2*torch.sigmoid(result)-1
    return result
