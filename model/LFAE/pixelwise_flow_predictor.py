...
class PixelwiseFlowPredictor(nn.Module):
    ...
    def create_heatmap_representations(self, source_image, driving_region_params, source_region_params):
        ...
        # print('region_var',self.region_var)
        # print("driving_region_",driving_region_params['covar'])
        covar = self.region_var if not self.use_covar_heatmap else driving_region_params['covar']
        # print('create_covar',covar)
        ...
        zeros = torch.zeros(heatmap.shape[0], 1, spatial_size[0], spatial_size[1]).to(heatmap.device)
    ...
    def create_sparse_motions(self, source_image, driving_region_params, source_region_params, bg_params=None):
        ...
        identity_grid = make_coordinate_grid((h, w), type=source_region_params['shift'].type(), device=source_image.device)
        identity_grid = identity_grid.view(1, 1, h, w, 2)
        coordinate_grid = identity_grid - driving_region_params['shift'].view(bs, self.num_regions, 1, 1, 2).to(source_image.device)
        if 'affine' in driving_region_params:
            affine = torch.matmul(source_region_params['affine'], torch.inverse(driving_region_params['affine'])).to(source_image.device)
        ...
        driving_to_source = coordinate_grid + source_region_params['shift'].view(bs, self.num_regions, 1, 1, 2).to(source_image.device)
        if bg_params is None:
            bg_grid = identity_grid.repeat(bs, 1, 1, 1, 1).to(source_image.device)
        else:
            bg_grid = identity_grid.repeat(bs, 1, 1, 1, 1).to(source_image.device)
            bg_grid = to_homogeneous(bg_grid)
            bg_grid = torch.matmul(bg_params.view(bs, 1, 1, 1, 3, 3).to(source_image.device), bg_grid.unsqueeze(-1)).squeeze(-1)
    ...
    def create_deformed_source_image(self, source_image, sparse_motions):
        ...
        # sparse_motions = sparse_motions.clamp(-1, 1)
    ...
    def forward(self, source_image, driving_region_params, source_region_params, bg_params=None):
        ...
        heatmap_representation = self.create_heatmap_representations(source_image, driving_region_params,
                                                                     source_region_params).to(source_image.device)
        sparse_motion = self.create_sparse_motions(source_image, driving_region_params,
                                                   source_region_params, bg_params=bg_params).to(source_image.device)
        # if torch.isnan(sparse_motion).any():
        #     print('the sparse_motion have nan')
        #     return
        deformed_source = self.create_deformed_source_image(source_image, sparse_motion).to(source_image.device)
        # if torch.isnan(heatmap_representation).any():
        #     print('find heatmap_representation NAN')
        #     return
        # if torch.isnan(deformed_source).any():
        #     print('find deformed_source NanN')
        #     return
        ...
        #     print('find predictor_input NAN')
        #     return
